...
public:
    ...
    const MaxCliqueSolution& GetClique() const {
        return best_clique;
    }
...
{
    if (argc != 4) {
        std::cout << "Usage: ./max_clique_tabu_search <INPUT-FILES-DIRECTORY> <NUM-ITERATIROS> <RANDOMIZATION>\n";
        exit(0);
    }
    ...
    std::ofstream fout("max_clique_tabu_search.csv");
    ...
    {   
        ...
        MaxCliqueTabuSearch problem(graphPtr);
        clock_t start = clock();
        problem.RunSearch(iterations, randomization);
        if (!problem.CheckCorrectness())
        {
        ...
        std::cout << inputFileName << ", result - " << problem.GetClique().size() << ", time - " << double(clock() - start) / CLOCKS_PER_SEC << '\n';
    }
